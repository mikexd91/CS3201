...
bool UsesClause::evaluateS1GenericS2Generic() {
	...
	for(auto i=allVars.begin(); i!=allVars.end(); i++) {
		set<int> usedByStmts = i->getUsedByStmts();
		set<string> usedByProc = i->getUsedByProc();
...
bool UsesClause::evaluateS1GenericS2Fixed(string s2) {
	...
	set<int> usedByStmts = var->getUsedByStmts();
	set<string> usedByProc = var->getUsedByProc();
